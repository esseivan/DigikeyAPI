...
namespace ApiClient
{
    public class ApiClientService
    {
        ...
        public async Task<string> ProductDetails(string digiKeyPartNumber, string include = null)
        {
            string encodedPartNumber = HttpUtility.UrlEncode(digiKeyPartNumber);
            var resourcePath = string.Format("/Search/v3/Products/{0}", encodedPartNumber);
            if (include != null)
                resourcePath += "?Includes=" + include;

            await ResetExpiredAccessTokenIfNeeded();
            var getResponse = await GetAsync(resourcePath);

            var t = await GetServiceResponse(getResponse);
        ...
        protected async Task<string> GetServiceResponse(HttpResponseMessage response)
        {
            ...
            else
            {
                ...
                Console.WriteLine("  Request Message: {0}", response.RequestMessage);
